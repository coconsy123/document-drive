        ...
        if action == "add" and request.method == "GET":
            ...
            with transaction.atomic():
                ...
                if t:
                    ...
                    date_of_coverage = request.POST.get('date_of_coverage')
                    ...
                    contract_files = ContractFiles.objects.create(title=title,description=description,category_type=category_type,division_type=division_type,section_type=section_type,created_by=request.user, remarks=remarks,date_of_coverage=date_of_coverage,status="Pending")
    ...
    if action == "generate-report":
        if request.method == "GET":
            ...
            if end_date:
                keyword.append(f"end_date={end_date.strftime('%Y-%m-%d')}")
            context['keyword'] = "&".join(keyword)
            return render(request, 'backend/contract_files/reports/partial-reports.html', context )
